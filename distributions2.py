...
for skip in skips:
    ...
    d[(f'{int(3000 / skip)}', 'Physics guided architecture')] = hy.values
    d[(f'{int(3000 / skip)}', 'Physics guided loss')] = loss.values
    d[(f'{int(3000 / skip)}', 'Physics guided initialization')] = trans.values
...
hy = df[df['variable_1'].isin(['Baseline', 'Physics guided architecture'])]
loss = df[df['variable_1'].isin(['Baseline', 'Physics guided loss'])]
trans = df[df['variable_1'].isin(['Baseline', 'Physics guided initialization'])]
print(hy)

a = sns.violinplot(x="variable_0", y="value", hue="variable_1", inner="stick", scale='width', data=hy, palette=[pal[0], pal[2]], linewidth=0.5, cut=0)
b = sns.stripplot(x="variable_0", y="value", hue="variable_1", data=hy, jitter=False, dodge=True, palette=['k', 'k'], ec="black", alpha=0.7, linewidth=.5, size=3, label='')
handles, labels = a.get_legend_handles_labels()
plt.legend(handles[0:2], labels[0:2], title=None)
plt.ylabel('Mean Square Error')
plt.xlabel('Training Data Points')
plt.savefig('plots/hy_vio.png', bbox_inches="tight")
plt.show()
